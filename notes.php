...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($_POST["select2"])) {
    ...
    //echo "string";
    $tags_arr = $_POST['select2'];
    
     $tags = $tags_arr[0];
   for ($i=1; $i < sizeof($tags_arr); $i++) {
     $tags.=",".$tags_arr[$i];
  }
} //echo "Tags:";
    //echo $tags;
  ...
  $comment_possible = $_POST["comment_possible"];
  //echo "comment_possible:";
  //echo $comment_possible;
...
insert_notes($original_user,$type,$repeat,$day,$start_time,$end_time,$start_date,$end_date,$latitude,$longitude,$Description,$visibility,$radius,$comment_possible,$tags);
...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">  
  ...
  <br><br>
  Comment_possible:
  <select  name="comment_possible" id="comment_possible" value="0"> 
...
?>
  <select name="select2[]" multiple="multiple" tabindex="1">
           <?php
